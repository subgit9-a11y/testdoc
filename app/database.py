...
class SupabaseManager:
    ...
    async def save_chat_message(self, *args, **kwargs) -> Any:
        ...
        try:
            if chat_data is not None:
                from app.security.encryption import field_encryptor
                encrypted_data = field_encryptor.encrypt_fields(chat_data, fields={'user_message', 'assistant_response'})
                res = self.client.table("astra_chat_history").insert(encrypted_data).execute()
            ...
            # AUTO-SESSION: If session_id is missing but we have user_id, 
            # we must find or create a session to satisfy the NOT NULL constraint.
            if not session_id and user_id:
                try:
                    # Try to get latest active session
                    sessions = self.client.table("astra_chat_sessions").select("id").eq("user_id", user_id).order("updated_at", desc=True).limit(1).execute()
                    if sessions.data:
                        session_id = sessions.data[0]["id"]
                    else:
                        # Create a new default session
                        session_id = await self.create_chat_session(user_id)
                except Exception as e:
                    logger.warning(f"Failed to auto-resolve session for {user_id}: {e}")
            ...
            # If we still have no session_id, we cannot insert (Supabase constraint)
            if not session_id:
                logger.error(f"Cannot save chat message: Missing session_id and couldn't create one for user {user_id}")
                return chat_data if chat_data else False
            ...
            from app.security.encryption import field_encryptor
            encrypted_msg = field_encryptor.encrypt_fields(msg, fields={'user_message', 'assistant_response'})
            res = self.client.table("astra_chat_history").insert(encrypted_msg).execute()
            # Update session
    ...
    async def get_chat_history(self, session_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if res.data:
                from app.security.encryption import field_encryptor
                return [field_encryptor.decrypt_fields(m, fields={'user_message', 'assistant_response'}) for m in res.data]
            return []
        except Exception as e:
    ...
    async def create_consultation(self, consult_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            from app.security.encryption import field_encryptor
            encrypted_consult = field_encryptor.encrypt_fields(consult_data, fields={'notes', 'diagnosis', 'symptoms'})
            res = self.client.table("consultations").insert(encrypted_consult).execute()
    ...
    async def get_patient_consultations(self, patient_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if res.data:
                from app.security.encryption import field_encryptor
                return [field_encryptor.decrypt_fields(c, fields={'notes', 'diagnosis', 'symptoms'}) for c in res.data]
            return []
        except Exception as e:
    ...
    async def get_latest_astra_fill(self, user_id: str) -> Optional[Dict[str, Any]]:
        """Get the most recent Astra Fill extraction for a user"""
        if not self.client: return None
        try:
            res = self.client.table("astra_fill_extractions").select("*").eq("user_id", user_id).order("created_at", desc=True).limit(1).execute()
            return res.data[0] if res.data else None
        except Exception as e:
            logger.error(f"Get latest astra fill error: {e}")
            return None

    async def get_astra_fill_history(self, user_id: str, limit: int = 20) -> List[Dict[str, Any]]:
        """Get Astra Fill extraction history for a user"""
        if not self.client: return []
        try:
            res = self.client.table("astra_fill_extractions").select("*").eq("user_id", user_id).order("created_at", desc=True).limit(limit).execute()
            return res.data or []
        except Exception as e:
            logger.error(f"Get astra fill history error: {e}")
            return []
