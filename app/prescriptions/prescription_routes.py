...
async def approve_prescription(
    ...
    """
    Approve a pending prescription and optionally trigger automation (Phase 3) (Doctor Only)
    """
    try:
...
async def reject_prescription(
    prescription_id: str, 
    reason: str = "Clinical mismatch",
    doctor: Dict[str, Any] = Depends(require_doctor_auth)
):
    """Reject a pending prescription (Phase 3) (Doctor Only)"""
    try:
        result = await prescription_service.update_prescription(
            prescription_id=prescription_id,
...
async def process_prescription(
    ...
    """
    Manually trigger prescription automation (Doctor Only)
