...
async def run_astra_scenario():
    ...
    print(f"   RUNNING: Processing prescription...", flush=True)
    ...
    print(f"   Result Success: {automation_result.get('success')}")
    if automation_result.get("shopify_order"):
        print(f"   Shopify Order: {automation_result['shopify_order']['invoice_url']}")
    else:
        print(f"   Shopify Order: NOT CREATED")
    
    if automation_result.get("pdf_url"):
        print(f"   EHR Stored (PDF): {automation_result['pdf_url']}")
    
    if automation_result.get("errors"):
        print(f"   Errors: {automation_result['errors']}")

    # 7. Asking AI Questions
    print(f"\n[STEP 7]: Questioning the AI", flush=True)
    user_query = "How should I apply the mahanarayana tailam?"
    print(f"   Patient: \"{user_query}\"")
    response = await pipeline.process_query(user_id=PATIENT_ID, message=user_query, history=[])
    print(f"   AI: \"{response}\"")

    # 8. Autopilot
    print(f"\n[STEP 8]: Autopilot Routing")
    route_query = "I want to talk to a doctor."
